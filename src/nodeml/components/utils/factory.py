"""Node factory utilities for the NodeML framework.

Provides helper functions to wrap common ML objects (sklearn estimators,
torch ``nn.Module`` subclasses, plain callables) into fully registered NodeML
pipeline nodes with minimal boilerplate.

Example — wrapping an sklearn estimator::

    from nodeml.components.utils.factory import create_sklearn_model_node, PortSpec
...
Example — wrapping a torch nn.Module::
    ...
    from nodeml.components.utils.factory import (
...
class PortSpec:
    ...
    def to_port(self) -> Port:
        """Convert to a full :class:`~nodeml.core.nodes.node.Port`."""
        return Port(
...
def _build_port_dict(specs: dict[str, PortSpec]) -> dict[str, Port]:
    """Convert a mapping of :class:`PortSpec` to NodeML :class:`Port` objects."""
...
def create_sklearn_model_node(
    ...
    *,
    ...
    """Create a NodeML Model node wrapping an sklearn estimator.
...
def create_torch_model_node(
    ...
    *,
    ...
    """Create a NodeML Model node wrapping a torch ``nn.Module``.
...
def create_callable_transform_node(
    ...
    *,
    ...
    """Create a NodeML TransformNode wrapping plain callables.
