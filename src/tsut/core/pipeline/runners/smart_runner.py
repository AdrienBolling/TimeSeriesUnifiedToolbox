...
class TabularSmartRunner(
    ...
    def evaluate(self) -> dict[str, Any]:
        ...
        last_node_w_metrics = self._get_execution_order()[
            -1
        ]  # The last node in the execution order with metrics is supposed to be the one that outputs the metrics.
    ...
    def _call_node(self, node_name: str) -> dict[str, TabularData]:
        ...
        if (
            self.node_objects[node_name].config.node_type == NodeType.METRIC
    ...
    def _execute_node(
        ...
        for edge in incoming_edges:
            ...
            inputs.update(
                {
                    target_key: self._runtime_typecheck_edge(
                        node, pred_node, pred_output, target_key, source_key
                    )
    ...
    def _runtime_typecheck_edge(
        ...
        target_node: Node,
        source_node: Node,
        source_output: dict[str, TabularData],
        target_key: str,
        source_key: str,
    ) -> TabularData:
        ...
        def runtime_type_check(data: TabularData) -> TabularData:
            if isinstance(data, target_jaxtyping_type):  # pyright: ignore[reportArgumentType] We ignore because it's runtime type checking
                return data
            msg = f"Data type mismatch between source node '{source_node}' and target node '{target_node}' on edge with source port '{source_key}' and target port '{target_key}'. Expected type: {target_jaxtyping_type}, but got type: {type(data)}. (Expected source jaxtyping type: {self._get_jaxtyping_type_from_port(source_port)}). Data: shape - {data.shape}, dtype - {data.dtype}"
            raise TypeError(msg)
    ...
    def _get_node_outputs(
        self, node_name: str, inputs: dict[str, tuple[ArrayLike, TabularDataContext]]
    ) -> dict[str, TabularData]:
